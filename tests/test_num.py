...
def test_fft2(backend: str):
    ...
    # point input, f = 25 delta(x) delta(y)
    a = xp.pad(xp.array([[25.]], dtype=numpy.float32), ((2, 2), (2, 2)))

    # even input, so output is real
    # delta function input, so output is constant
    # normalized, so intensity in = mass out
    ...
    assert_array_almost_equal(
        to_numpy(fft2(a)),
        numpy.pad([[1.+1.j]], ((0, 4), (0, 4))).astype(numpy.complex64)
    )
...
def test_ifft2(backend: str):
    ...
    a = xp.pad(xp.array([[1.]], dtype=numpy.float32), ((0, 4), (0, 4)))
