...
def initialize(sim_size: int = 128) -> t.Tuple[ReconsPlan, Patterns, ReconsState]:
    plan = ReconsPlan.from_data({
        ...
        'raw_data': {
            'type': 'empad',
            'path': "~/Downloads/si-final4/Si_110_Sn_300kV_conv25_defocus20_tds/Si_110_Sn_300kV_conv25_defocus20_tds_199.70_dstep0.8_x80_y80_4DSTEM.raw",
            'diff_step': 0.8,
            'kv': 300.0
        },
        'post_load': [
            {'type': 'poisson', 'scale': 14.5e6},
        ],
        'init_probe': {'type': 'focused', 'conv_angle': 25.0, 'defocus': 200.0},
        'init_object': 'random',
        'init_scan': {'type': 'raster', 'shape': (80, 80), 'step_size': 0.3},
        'post_init': [],
...
if __name__ == '__main__':
    ...
    for sim_size, grouping in itertools.product([128, 192], [16, 32, 64, 128]):
        print(f"Running {sim_size}x{sim_size}, grouping {grouping}", file=sys.stderr)
        try:
    ...
    #df = polars.DataFrame(rows, orient='row')
    #df.write_ndjson(sys.stdout)  # type: ignore