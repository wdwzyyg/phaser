...
def load_files(paths: t.Iterable[t.Union[str, Path]], inner: float, outer: float) -> t.Iterable[t.Tuple[Path, AnyMetadata, NDArray[numpy.float32]]]:
    queue: Queue[t.Tuple[Path, AnyMetadata, NDArray[numpy.float32]]] = Queue(1)
    finished = Event()
    ...
    for (meta_path, meta, virtual_img) in load_files(paths, inner, outer):
        ...
        while True:
            ...
            with open(new_meta_path, 'w') as f:
                f.write(new_meta.json(indent=4, exclude=set('path')))

            console.print(f"New metadata written to '{new_meta_path}'!")
            break


def calc_drift_one(console: Console, meta: AnyMetadata, virtual_img: NDArray[numpy.float32], params: t.Dict[str, t.Any]):
    print(meta)
    ...
    def on_press(event: KeyEvent):
        ...
        r = numpy.diag(numpy.sign(numpy.diagonal(r))) @ r  # flip to ensure diagonals are positive. These are absorbed into `q`.
